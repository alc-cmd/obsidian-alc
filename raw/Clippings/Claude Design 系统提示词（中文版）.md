...
// 在 components.jsx 文件末尾： Object.assign(window, { Terminal, Line, Spacer, Gray, Blue, Green, Bold, // ... 所有需要共享的组件 });
...
<script> (async () => { const text = await window.claude.complete("Summarize this: ..."); // 或者传 messages 数组： const text2 = await window.claude.complete({ messages: \[{ role: 'user', content: '...' }\], }); })(); </script>
